...
Threads you're no longer working on can be archived by hovering over them in the sidebar and clicking the archive icon, or selecting them and pressing {#kb agent::ArchiveSelectedThread}. The Thread History holds all threads for the current workspace, including archived threads, sorted chronologically. You can restore them at any time.
