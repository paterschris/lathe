...
pub struct LocalSnapshot {
    ...
    /// to relative paths within the worktree, used to translate FSEvents
...
/// Builds a gitignore whose patterns resolve against `root` rather than the
/// directory holding the file. `.git/info/exclude` lives under the git dir but
/// its rules apply to the work directory, and `IgnoreStack` only consults an
/// entry for paths under `Gitignore::path`.
