...
pub(super) fn spawn_stdio(
    ...
    // The child inherits Lathe's own environment by default, so removed keys
    // must be explicitly unset: omitting them from `environment` is not enough
    // to stop an ambient API key from reaching the agent.
    ...
    // `lines()` splits on `\n` but keeps the `\r` of a CRLF pair. ACP messages
    // are newline-delimited JSON, so a stray `\r` breaks parsing for any agent
    // that emits CRLF (common on Windows).
