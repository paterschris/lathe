...
async fn test_file_history_action_uses_focused_project_panel_selection(
    ...
    workspace.read_with(&*cx, |workspace, cx| {
        let views = workspace
            .items_of_type::<git_ui::file_history_view::FileHistoryView>(cx)
            .collect::<Vec<_>>();
        assert_eq!(views.len(), 1);
        // The tab text encodes which file's history was opened, proving the
        // focused project panel selection (tracked1.txt) was used rather than
        // the active editor's file (tracked2.txt).
        assert_eq!(
            views[0].read(cx).tab_content_text(0, cx).to_string(),
            format!(
                "History: {}",
                project_panel_repo_path.file_name().unwrap_or_default()
            )
        );
    });
}
