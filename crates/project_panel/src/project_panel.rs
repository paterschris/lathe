        ...
        workspace.register_action(|workspace, _: &git::FileHistory, window, cx| {
            ...
            if let Some(panel) = workspace.panel::<ProjectPanel>(cx) {
                ...
                if let Some(project_path) = maybe_project_path {
                    ...
                    // When the project panel owns a focused file selection it
                    // consumes `git::FileHistory`: do not fall back to the active
                    // editor even when the selected file is not in a git repo.
                    return;
                }
            }
