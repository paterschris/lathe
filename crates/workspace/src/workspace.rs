...
actions!(
    ...
    [
        ...
        SaveAs,
        /// Saves the current workspace (project folder list) as a portable .lathe-workspace file that can be reopened later.
        SaveWorkspaceAs,
        /// Saves the current workspace folder list to its existing .lathe-workspace file. Falls back to Save Workspace As when no file is associated yet.
        SaveWorkspace,
...
pub struct Workspace {
    ...
    portable_workspace_path: Option<PathBuf>,
...
impl Workspace {
    ...
    pub fn actions(&self, div: Div, window: &mut Window, cx: &mut Context<Self>) -> Div {
        ...
        self.add_workspace_actions_listeners(div, window, cx)
            ...
            .on_action(cx.listener(|workspace, _: &SaveWorkspaceAs, window, cx| {
                workspace
                    .save_workspace_as(window, cx)
                    .detach_and_prompt_err("Failed to save workspace", window, cx, |_, _, _| None);
            }))
            .on_action(cx.listener(|workspace, _: &SaveWorkspace, window, cx| {
                workspace.save_workspace(window, cx).detach_and_prompt_err(
...
mod tests {
    ...
    async fn test_window_title_format_path_variables(cx: &mut TestAppContext) {
        ...
        cx.executor().run_until_parked();
        assert_eq!(cx.window_title().as_deref(), Some("Lathe — root1, root2"));
        ...
        assert_eq!(
            cx.window_title().as_deref(),
            Some(format!("Lathe — root1, root2 — one — {expected_file_path}").as_str())
        );
    }
