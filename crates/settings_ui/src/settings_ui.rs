    ...
    fn reset_to_default_fn(
        ...
        return Some(Box::new(move |window, cx| {
            ...
            update_settings_file(
                ...
                cx,
                move |settings, cx| {
                    (this.write)(settings, value_to_set, cx);
                },
            )
        ...
        cx.open_window(
            WindowOptions {
                titlebar: Some(TitlebarOptions {
                    title: Some("Lathe — Settings".into()),
...
pub struct SettingsWindow {
    ...
    /// Directory paths of skills the user deleted in this session, hidden from
    /// the Skills page until the skill index confirms their removal.
...
impl SettingsWindow {
    fn new(
        ...
        cx.observe_global_in::<agent_skills::SkillIndex>(window, |this, _window, cx| {
            if let Some(skill_index) = cx.try_global::<agent_skills::SkillIndex>() {
        ...
        .on_confirm({
            move |new_text, window, cx| {
                update_settings_file(
                    ...
                    cx,
                    move |settings, _cx| {
                        (field.write)(settings, new_text.map(Into::into), _cx);
                    },
                )
        ...
        .on_click({
            move |state, window, cx| {
                ...
                update_settings_file(file.clone(), field.json_path, window, cx, move |settings, _cx| {
                    (field.write)(settings, Some(state.into()), _cx);
                })
                .log_err(); // todo(settings_ui) don't log err
            }
        })
        ...
        .on_change({
            move |value, window, cx| {
                ...
                update_settings_file(
                    ...
                    cx,
                    move |settings, _cx| {
                        (field.write)(settings, Some(value), _cx);
                    },
                )
                .log_err(); // todo(settings_ui) don't log err
            }
                ...
                font_picker(
                    ...
                    move |font_name, window, cx| {
                        update_settings_file(
                            ...
                            cx,
                            move |settings, _cx| {
                                (field.write)(settings, Some(font_name.to_string().into()), _cx);
                            },
                        )
                        .log_err(); // todo(settings_ui) don't log err
                    },
...
pub mod test {
    ...
    async fn test_skills_page_scope_switch_updates_displayed_skills(cx: &mut gpui::TestAppContext) {
        use agent_skills::{
            ProjectSkillGroup, Skill, SkillIndex, SkillScopeId, SkillSource,
