...
use editor::{
    Editor, EditorEvent, EditorMode, MinimapVisibility, HiddenUnstagedDiffHunkRenderer,
    SizingBehavior,
};
use gpui::{
...
fn diff_editor_text_style_refinement(window: &Window, cx: &mut App) -> TextStyleRefinement {
    TextStyleRefinement {
        font_size: Some(
            TextSize::Small
                .rems(cx)
                .to_pixels(ThemeSettings::get_global(cx).agent_ui_font_size_in_window(window, cx))
                .into(),
        ),
        ..Default::default()
    }
