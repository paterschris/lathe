mod parallel_agent_announcement;

use auto_update::{AutoUpdater, release_notes_url};
use editor::{Editor, MultiBuffer};
use gpui::{
    App, DismissEvent, Entity, TaskExt, Window, actions, prelude::*,
};
...
fn show_update_notification(cx: &mut App) {
    ...
    if let Some(content) = parallel_agent_announcement::announcement_for_version(&version, cx) {
        show_app_notification(
            NotificationId::unique::<UpdateNotification>(),
            cx,
            move |cx| {
                cx.new(|cx| {
                    parallel_agent_announcement::AnnouncementToastNotification::new(
                        content.clone(),
                        cx,
                    )
                })
