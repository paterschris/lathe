...
pub fn clear_window_font_overrides(cx: &mut App, window_id: WindowId) {
    ...
    if cx.has_global::<AgentBufferFontSize>() {
        cx.default_global::<AgentBufferFontSize>().0.remove(&window_id);
    }
...
impl ThemeSettings {
    /// Returns the buffer font size as configured in settings, ignoring any
    /// per-window zoom adjustments. Use [`Self::buffer_font_size_in_window`]
    /// to honor per-window CMD+/- zoom.
    pub fn buffer_font_size(&self, _cx: &App) -> Pixels {
        clamp_font_size(self.buffer_font_size)
    }

    /// Returns the UI font size as configured in settings, ignoring any
    /// per-window zoom adjustments. Use [`Self::ui_font_size_in_window`] to
    /// honor per-window CMD+/- zoom.
...
pub fn reset_ui_font_size(window: &mut Window, cx: &mut App) {
    let id = window.window_handle().window_id();
    if cx.has_global::<UiFontSize>()
        && cx.default_global::<UiFontSize>().0.remove(&id).is_some()
    {
        window.refresh();
    }
}
...
pub fn adjust_git_commit_buffer_font_size(
    ...
    let id = window.window_handle().window_id();
    let current =
        ThemeSettings::get_global(cx).git_commit_buffer_font_size_in_window(window, cx);
    let new_size = clamp_font_size(f(current));
    cx.default_global::<GitCommitBufferFontSize>()
        .0
        .insert(id, new_size);
