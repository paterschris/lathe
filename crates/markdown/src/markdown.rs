...
| `a\|b` |";
    ...
    fn test_editor_zoom_does_not_affect_markdown_preview(cx: &mut TestAppContext) {
        ...
        // Editor zoom is per-window in upstream, so drive it through a window and
        // read the zoomed value via buffer_font_size_in_window.
        let (_, cx) = cx.add_window_view(|_, _| TestWindow);
        cx.update(|window, cx| {
            ...
            assert_eq!(
                ThemeSettings::get_global(cx).buffer_font_size_in_window(window, cx),
                px(19.0)
            );
            assert_eq!(
