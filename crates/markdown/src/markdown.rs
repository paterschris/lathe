...
| `a\|b` |";
    ...
    fn test_ui_zoom_does_not_affect_markdown_preview(cx: &mut TestAppContext) {
        ...
        // UI zoom is per-window in the fork, so drive it through a window and
        // read the zoomed value via ui_font_size_in_window.
        let (_, cx) = cx.add_window_view(|_, _| TestWindow);
        cx.update(|window, cx| {
            let before = ThemeSettings::get_global(cx).markdown_preview_font_size(cx);
            assert_eq!(before, px(16.0));

            theme_settings::adjust_ui_font_size(window, cx, |size| size + px(3.0));

            assert_eq!(
                ThemeSettings::get_global(cx).ui_font_size_in_window(window, cx),
                px(19.0)
            );
            assert_eq!(
