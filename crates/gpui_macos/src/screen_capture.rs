...
pub(crate) fn get_sources(
    ...
    // SCShareableContent reports window frames in points; the completion
    // handler runs off the main thread, so read the scale factor up front.
    let backing_scale = NSScreen::mainScreen(marker)
    ...
    let handler = RcBlock::new(
        move |content: *mut SCShareableContent, error: *mut NSError| {
            ...
            let result = if let Some(error) = unsafe { error.as_ref() } {
                ...
                let mut result = unsafe { content.displays() }
                    .into_iter()
                    .map(|display| {
                        // SAFETY: Marked unsafe conservatively by objc2
                ...
                for window in unsafe { content.windows() } {
                    let source = unsafe {
                        screen_capture_window::window_source(
                            window,
                            own_bundle.as_deref(),
                            backing_scale,
                        )
