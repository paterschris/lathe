...
use gpui::{
    ...
    TaskExt, TextStyle, UniformListScrollHandle, WeakEntity, actions, anchored, deferred, point,
    size, uniform_list,
};
...
impl GitPanel {
                        ...
                        tree_state.build_tree_entries(section, entries, &mut seen_directories)
                    {
                        ...
                        if let Some(path) = hunk_target {
                            ...
                            for h in hunk_entries {
                                push_entry(self, h, true, Some(&mut tree_state.logical_indices));
                            }
                        }
                    }
                }
    ...
    fn render_commit_history(
        ...
        let shas = self.commit_history_shas.clone().unwrap_or_default();
        if shas.is_empty() {
            return None;
        }
