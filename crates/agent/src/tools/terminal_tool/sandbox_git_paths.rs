    ...
    pub(crate) fn from_project(project: &Project, cx: &App) -> Self {
        ...
        for repository in project.git_store().read(cx).repositories().values() {
            ...
            let repository_paths = SandboxGitRepositoryPaths {
                ...
                // `RepositorySnapshot` doesn't carry a separate dot-git path here,
                // so derive the worktree `.git` entry from the work directory; this
                // matches the value upstream stores for a normal checkout.
                dot_git_abs_path: repository.work_directory_abs_path.join(".git"),
