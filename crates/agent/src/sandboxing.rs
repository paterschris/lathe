...
pub fn sandbox_git_dirs(project: &Project, cx: &App) -> Vec<PathBuf> {
    ...
    for repository in project.git_store().read(cx).repositories().values() {
        ...
        // `RepositorySnapshot` doesn't carry a separate dot-git path here, so
        // derive the worktree `.git` entry from the work directory.
        git_dirs.push(repository.work_directory_abs_path.join(".git"));
