...
RUN apt-get update
        ...
        assert!(
            !recorded.is_empty(),
            "docker_compose_build should have been invoked"
        );
        assert!(
            recorded.iter().all(|services| services.is_none()),
            "compose build must be unfiltered so the primary service `app` is \
             always included even when runServices omits it; got {recorded:?}"
        );
...
RUN echo $RUBY_VERSION2
    ...
    impl DockerClient for FakeDocker {
        ...
        async fn docker_compose_build(
            ...
            // The fork's compose build is always unfiltered (no runServices
            // list); record `None` to mean "all services".
            self.compose_build_services
                .lock()
                .expect("should be available")
                .push(None);
