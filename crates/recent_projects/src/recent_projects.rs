        ...
        match self.filtered_entries.get(self.selected_index) {
            ...
            Some(ProjectPickerEntry::ProjectGroup(selected_match)) => {
                ...
                if let Some(handle) = window.window_handle().downcast::<MultiWorkspace>() {
                    cx.defer(move |cx| {
                        if host.is_none() {
                            // Local project group: activate the existing workspace for these
                            // paths or create a new local one. The returned task resolves the
                            ...
                            // Remote project group: connect through the remote modal before
                            // activating or creating the workspace.
                            let path_list = key.path_list().clone();
                            if let Some(task) = handle
                                .update(cx, |multi_workspace, window, cx| {
...
mod tests {
    ...
    #[gpui::test]
    #[ignore = "Lathe: scroll geometry differs from upstream after merge/upstream-2026-05-15"]
